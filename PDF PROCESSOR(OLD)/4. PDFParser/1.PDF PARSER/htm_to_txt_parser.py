...
CURRENT_PATH = Path(Path.cwd(), "EXTRACTED_FILES")
...
def extract_text(filename):  
    ...
    file_path = Path(Path.cwd(), "EXTRACTED_FILES", input_file_name, filename)
    ...
    with open(str(file_path), "r", encoding="UTF-8") as f:        
        ...
        # Save data as json
        output_txt_path = Path(Path.cwd(), "EXTRACTED_FILES", input_file_name)
        output_txt_path.mkdir(parents=True, exist_ok=True) 
        for i, page in enumerate(pages_text[5:6]):
            text_file_path = Path(output_txt_path,  input_file_name + "-page-" + str(i+1) + ".csv")  
            df = pd.DataFrame({"page" : page})
